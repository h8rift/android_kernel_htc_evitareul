/*
 * arch/arm/mach-tegra/board-evitareul-panel.c
 *
...
#define evitareul_lvds_shutdown	TEGRA_GPIO_PL2
#define evitareul_hdmi_hpd		TEGRA_GPIO_PN7

#define evitareul_dsi_panel_reset	TEGRA_GPIO_PW0

#define evitareul_lcd_2d_3d		TEGRA_GPIO_PH1
#define EVITAREUL_STEREO_3D		0
#define EVITAREUL_STEREO_2D		1

#define evitareul_lcd_swp_pl		TEGRA_GPIO_PH2
#define EVITAREUL_STEREO_LANDSCAPE	0
#define EVITAREUL_STEREO_PORTRAIT	1

#ifdef CONFIG_TEGRA_DC
static struct regulator *evitareul_dsi_reg = NULL;
static bool dsi_regulator_status = 0;
static struct regulator *evitareul_lcd_reg = NULL;
static struct regulator *v_lcm_3v = NULL;
static struct regulator *v_lcmio_1v8 = NULL;

static struct regulator *evitareul_hdmi_reg = NULL;
static struct regulator *evitareul_hdmi_pll = NULL;
...
static tegra_dc_bl_output evitareul_bl_output_measured_a02 = {
...
static tegra_dc_bl_output evitareul_bl_output_measured_a03 = {
	0, 1, 2, 3, 4, 5, 6, 7,
...
static int evitareul_backlight_notify(struct device *unused, int brightness)
{
...
static int evitareul_disp1_check_fb(struct device *dev, struct fb_info *info);
 ...
 */
static struct platform_tegra_pwm_backlight_data evitareul_disp1_backlight_data = {
	.which_dc		= 0,
	...
	.max_brightness		= 255,
	.dft_brightness		= 142,
	.notify		= evitareul_backlight_notify,
	.period			= 0xFF,
	.clk_div		= 20,
	.clk_select		= 0,
...
static struct platform_device evitareul_disp1_backlight_device = {
	...
	.id	= -1,
	.dev	= {
		.platform_data = &evitareul_disp1_backlight_data,
	},
};

#ifdef CONFIG_TEGRA_DC
static int evitareul_hdmi_vddio_enable(void)
{
	return 0;
}

static int evitareul_hdmi_vddio_disable(void)
{
	return 0;
}

static int evitareul_hdmi_enable(void)
{
	REGULATOR_GET(evitareul_hdmi_reg, "avdd_hdmi");
	regulator_enable(evitareul_hdmi_reg);

	REGULATOR_GET(evitareul_hdmi_pll, "avdd_hdmi_pll");
	regulator_enable(evitareul_hdmi_pll);

failed:
	return 0;
}

static int evitareul_hdmi_disable(void)
{

	regulator_disable(evitareul_hdmi_reg);
	regulator_put(evitareul_hdmi_reg);
	evitareul_hdmi_reg = NULL;

	regulator_disable(evitareul_hdmi_pll);
	regulator_put(evitareul_hdmi_pll);
	evitareul_hdmi_pll = NULL;

	return 0;
}
static struct resource evitareul_disp1_resources[] = {
	{
		.name	= "irq",
	...
	{
		.name	= "fbmem",
		.start	= 0,	/* Filled in by evitareul_panel_init() */
		.end	= 0,	/* Filled in by evitareul_panel_init() */
		.flags	= IORESOURCE_MEM,
	},
	{
		.name	= "dsi_regs",
...
static struct resource evitareul_disp2_resources[] = {
	{
		.name	= "irq",
...
static struct tegra_dc_sd_settings evitareul_sd_settings = {
	.enable = 0, /* Normal mode operation */
	.bl_device = &evitareul_disp1_backlight_device,
};

static struct tegra_fb_data evitareul_hdmi_fb_data = {
	.win		= 0,
	.xres		= 1280,
	.yres		= 720,
	.bits_per_pixel	= 32,
...
static struct tegra_dc_out evitareul_disp2_out = {
	.type		= TEGRA_DC_OUT_HDMI,
	.flags		= TEGRA_DC_OUT_HOTPLUG_HIGH,
	.parent_clk	= "pll_d2_out0",

	.dcc_bus	= 3,
	.hotplug_gpio	= evitareul_hdmi_hpd,
	...
	.enable		= evitareul_hdmi_enable,
	.disable	= evitareul_hdmi_disable,
	.postsuspend	= evitareul_hdmi_vddio_disable,
	.hotplug_init	= evitareul_hdmi_vddio_enable,
};

static struct tegra_dc_platform_data evitareul_disp2_pdata = {
	.flags		= TEGRA_DC_FLAG_ENABLED,
	.default_out	= &evitareul_disp2_out,
	.fb		= &evitareul_hdmi_fb_data,
	.emc_clk_rate	= 300000000,
};

static int evitareul_dsi_panel_enable(void)
{
...
static int bridge_reset(void)
{
	...
	if(g_display_on)
	{
		...
		regulator_enable(evitareul_dsi_reg);
...
static int evitareul_dsi_panel_disable(void)
{
	int err = 0;
...
static void evitareul_stereo_set_mode(int mode)
{
	switch (mode) {
	case TEGRA_DC_STEREO_MODE_2D:
		/*gpio_set_value(TEGRA_GPIO_PH1, EVITAREUL_STEREO_2D);*/
		break;
	case TEGRA_DC_STEREO_MODE_3D:
		/*gpio_set_value(TEGRA_GPIO_PH1, EVITAREUL_STEREO_3D);*/
		break;
	}
}

static void evitareul_stereo_set_orientation(int mode)
{
	switch (mode) {
	case TEGRA_DC_STEREO_LANDSCAPE:
		/*gpio_set_value(TEGRA_GPIO_PH2, EVITAREUL_STEREO_LANDSCAPE);*/
		break;
	case TEGRA_DC_STEREO_PORTRAIT:
		/*gpio_set_value(TEGRA_GPIO_PH2, EVITAREUL_STEREO_PORTRAIT);*/
		break;
	}
}

#ifdef CONFIG_TEGRA_DC
static int evitareul_dsi_panel_postsuspend(void)
{
	int err = 0;
...
struct tegra_dsi_out evitareul_dsi = {
	.n_data_lanes = 2,
...
static struct tegra_stereo_out evitareul_stereo = {
	.set_mode		= &evitareul_stereo_set_mode,
	.set_orientation	= &evitareul_stereo_set_orientation,
};

#ifdef CONFIG_TEGRA_DC
static struct tegra_dc_mode evitareul_dsi_modes[] = {
	{
		.pclk = 20000000,
		.h_ref_to_sync = 4,
		.v_ref_to_sync = 1,
...
static struct tegra_fb_data evitareul_dsi_fb_data = {
	.win		= 0,
	.xres		= 720,
	.yres		= 1280,
	.bits_per_pixel	= 32,
...
static struct tegra_dc_out evitareul_disp1_out = {
	.align		= TEGRA_DC_ALIGN_MSB,
	.order		= TEGRA_DC_ORDER_RED_BLUE,
	.sd_settings	= &evitareul_sd_settings,

	.flags		= DC_CTRL_MODE,

	.type		= TEGRA_DC_OUT_DSI,

	.modes		= evitareul_dsi_modes,
	.n_modes	= ARRAY_SIZE(evitareul_dsi_modes),

	.dsi		= &evitareul_dsi,
	.stereo		= &evitareul_stereo,

	.enable		= evitareul_dsi_panel_enable,
	.disable	= evitareul_dsi_panel_disable,
	.postsuspend	= evitareul_dsi_panel_postsuspend,
...
static struct tegra_dc_platform_data evitareul_disp1_pdata = {
	.flags		= TEGRA_DC_FLAG_ENABLED,
	.default_out	= &evitareul_disp1_out,
	.emc_clk_rate	= 204000000,
	.fb		= &evitareul_dsi_fb_data,
};

static struct nvhost_device evitareul_disp1_device = {
	.name		= "tegradc",
	.id		= 0,
	.resource	= evitareul_disp1_resources,
	.num_resources	= ARRAY_SIZE(evitareul_disp1_resources),
	.dev = {
		.platform_data = &evitareul_disp1_pdata,
	},
};

static int evitareul_disp1_check_fb(struct device *dev, struct fb_info *info)
{
	return info->device == &evitareul_disp1_device.dev;
}

static struct nvhost_device evitareul_disp2_device = {
	.name		= "tegradc",
	.id		= 1,
	.resource	= evitareul_disp2_resources,
	.num_resources	= ARRAY_SIZE(evitareul_disp2_resources),
	.dev = {
		.platform_data = &evitareul_disp2_pdata,
	},
};
#endif

#if defined(CONFIG_TEGRA_NVMAP)
static struct nvmap_platform_carveout evitareul_carveouts[] = {
	[0] = NVMAP_HEAP_CARVEOUT_IRAM_INIT,
	[1] = {
		.name		= "generic-0",
		.usage_mask	= NVMAP_HEAP_CARVEOUT_GENERIC,
		.base		= 0,	/* Filled in by evitareul_panel_init() */
		.size		= 0,	/* Filled in by evitareul_panel_init() */
		.buddy_size	= SZ_32K,
	},
};

static struct nvmap_platform_data evitareul_nvmap_data = {
	.carveouts	= evitareul_carveouts,
	.nr_carveouts	= ARRAY_SIZE(evitareul_carveouts),
};

static struct platform_device evitareul_nvmap_device = {
	.name	= "tegra-nvmap",
	.id	= -1,
	.dev	= {
		.platform_data = &evitareul_nvmap_data,
	},
};
#endif

static struct platform_device *evitareul_gfx_devices[] __initdata = {
#if defined(CONFIG_TEGRA_NVMAP)
	&evitareul_nvmap_device,
#endif
	&tegra_pwfm0_device,
};

static struct platform_device *evitareul_bl_devices[]  = {
	&evitareul_disp1_backlight_device,
};
...
int __init evitareul_panel_init(void)
{
	int err;
	...
	BUILD_BUG_ON(ARRAY_SIZE(evitareul_bl_output_measured_a03) != 256);
	BUILD_BUG_ON(ARRAY_SIZE(evitareul_bl_output_measured_a02) != 256);

	if (board_info.fab >= BOARD_FAB_A03) {
		evitareul_disp1_backlight_data.clk_div = 0x1D;
		bl_output = evitareul_bl_output_measured_a03;
	} else
		bl_output = evitareul_bl_output_measured_a02;

	evitareul_dsi.chip_id = tegra_get_chipid();
	evitareul_dsi.chip_rev = tegra_get_revision();

#if defined(CONFIG_TEGRA_NVMAP)
	evitareul_carveouts[1].base = tegra_carveout_start;
	evitareul_carveouts[1].size = tegra_carveout_size;
#endif

	tegra_gpio_enable(evitareul_hdmi_hpd);
	gpio_request(evitareul_hdmi_hpd, "hdmi_hpd");
	gpio_direction_input(evitareul_hdmi_hpd);

	tegra_gpio_enable(evitareul_lcd_2d_3d);
	gpio_request(evitareul_lcd_2d_3d, "lcd_2d_3d");
	gpio_direction_output(evitareul_lcd_2d_3d, 0);
	evitareul_stereo_set_mode(evitareul_stereo.mode_2d_3d);

	tegra_gpio_enable(evitareul_lcd_swp_pl);
	gpio_request(evitareul_lcd_swp_pl, "lcd_swp_pl");
	gpio_direction_output(evitareul_lcd_swp_pl, 0);
	evitareul_stereo_set_orientation(evitareul_stereo.orientation);

#if !(DC_CTRL_MODE & TEGRA_DC_OUT_ONE_SHOT_MODE)
	tegra_gpio_enable(LCM_TE);
	gpio_request(evitareul_lcd_swp_pl, "lcd_te");
	gpio_direction_input(LCM_TE);
#endif

#ifdef CONFIG_HAS_EARLYSUSPEND
	evitareul_panel_early_suspender.suspend = evitareul_panel_early_suspend;
	...
	err = platform_add_devices(evitareul_gfx_devices,
				ARRAY_SIZE(evitareul_gfx_devices));

#if defined(CONFIG_TEGRA_GRHOST) && defined(CONFIG_TEGRA_DC)
	res = nvhost_get_resource_byname(&evitareul_disp1_device,
	...
	switch (PANEL_MASK(g_panel_id)) {
		case PANEL_ID_SHARP_HX_C3:
			evitareul_dsi.n_init_cmd = ARRAY_SIZE(dsi_init_sharp_hx_c3_cmd);
			evitareul_dsi.dsi_init_cmd = dsi_init_sharp_hx_c3_cmd;
		...
		case PANEL_ID_SHARP_HX_C4:
			...
			evitareul_dsi.dsi_init_cmd = dsi_init_sharp_hx_c4_cmd;
			evitareul_dsi.n_cabc_cmd = ARRAY_SIZE(hx_moving_mode_cmd);
			evitareul_dsi.dsi_cabc_moving_mode = hx_moving_mode_cmd;
			evitareul_dsi.dsi_cabc_still_mode = hx_still_mode_cmd;
		break;
		...
		case PANEL_ID_SHARP_HX:
			evitareul_dsi.n_init_cmd = ARRAY_SIZE(dsi_init_sharp_hx_c5_cmd);
			evitareul_dsi.dsi_init_cmd = dsi_init_sharp_hx_c5_cmd;
			evitareul_dsi.n_cabc_cmd = ARRAY_SIZE(hx_moving_mode_cmd);
			evitareul_dsi.dsi_cabc_moving_mode = hx_moving_mode_cmd;
			evitareul_dsi.dsi_cabc_still_mode = hx_still_mode_cmd;
		break;
		case PANEL_ID_SONY_NT_C1:
			...
			evitareul_dsi.dsi_init_cmd = dsi_init_sony_nt_c1_cmd;

			evitareul_dsi.n_osc_off_cmd = ARRAY_SIZE(osc_off_cmd);
			evitareul_dsi.osc_off_cmd = osc_off_cmd;
			evitareul_dsi.n_osc_on_cmd = ARRAY_SIZE(osc_on_cmd);
			evitareul_dsi.osc_on_cmd = osc_on_cmd;
		...
		case PANEL_ID_SONY:
			evitareul_dsi.n_init_cmd = ARRAY_SIZE(dsi_init_sony_nt_c2_cmd);
			evitareul_dsi.dsi_init_cmd = dsi_init_sony_nt_c2_cmd;
		...
		case PANEL_ID_SHARP_NT_C1:
			...
			evitareul_dsi.dsi_init_cmd = dsi_init_sharp_nt_c1_cmd;
			evitareul_dsi.refresh_rate = 60;
			evitareul_dsi_modes[0].h_front_porch = 29;
		...
		case PANEL_ID_SHARP_NT_C2:
			...
			evitareul_dsi.dsi_init_cmd = dsi_init_sharp_nt_c2_cmd;
		...
		case PANEL_ID_SHARP_NT_C2_9A:
			...
			evitareul_dsi.dsi_init_cmd = dsi_init_sharp_nt_c2_9a_cmd;
		...
		case PANEL_ID_SHARP:
			...
			evitareul_dsi.dsi_init_cmd = dsi_init_sharp_unknow_cmd;
		break;
		case PANEL_ID_AUO_NT_C2:
			evitareul_dsi.n_init_cmd = ARRAY_SIZE(dsi_init_auo_nt_c2_cmd);
			evitareul_dsi.dsi_init_cmd = dsi_init_auo_nt_c2_cmd;
		...
		case PANEL_ID_AUO:
			evitareul_dsi.n_init_cmd = ARRAY_SIZE(dsi_init_auo_nt_x7_cmd);
			evitareul_dsi.dsi_init_cmd = dsi_init_auo_nt_x7_cmd;
...
#if defined(CONFIG_TEGRA_GRHOST) && defined(CONFIG_TEGRA_DC)
	if (!err)
		err = nvhost_device_register(&evitareul_disp1_device);

	res = nvhost_get_resource_byname(&evitareul_disp2_device,
					 IORESOURCE_MEM, "fbmem");
	if (res) {
	...
	if (!err)
		err = nvhost_device_register(&evitareul_disp2_device);
#endif
	...
	if( (board_mfg_mode() == BOARD_MFG_MODE_OFFMODE_CHARGING) ) {
		if (board_zchg_mode() & 0x1)	/*off-mode charge with china sku, need to draw battery*/
			evitareul_disp1_backlight_data.backlight_status = BACKLIGHT_SKIP_WHEN_PROBE;
		else				/*off-mode charge without china sku, do not need to open backlight */
			evitareul_disp1_backlight_data.backlight_status = BACKLIGHT_DISABLE;
	}

	if (!err)
		err = platform_add_devices(evitareul_bl_devices,
				ARRAY_SIZE(evitareul_bl_devices));
